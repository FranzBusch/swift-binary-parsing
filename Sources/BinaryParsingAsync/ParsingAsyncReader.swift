...
where Upstream.ReadElement == UInt8 {
    ...
    /// to the body closure via an ``InputSpan``.
    ///
    /// Accumulates bytes from upstream until at least one value can be
    /// parsed. Passes all parsed values as an ``InputSpan`` to `body`.
    /// An empty span signals end of stream.
    ///
    /// - Parameters:
    ///   - maximumCount: The maximum number of values to parse before
    ///     yielding to the body closure.
    ///   - body: A closure that processes the parsed values.
    /// - Returns: The value returned by the body closure.
    /// - Throws: ``TruncatedStreamError`` if the stream ends with leftover
    ///   bytes that cannot form a complete value.
    public mutating func read<Return, Failure: Error>(
        maximumCount: Int,
        body: nonisolated(nonsending) (consuming InputSpan<Value>) async throws(Failure) -> Return
    ) async throws(EitherError<any Error, Failure>) -> Return {
        valueBuffer.removeAll()

        while valueBuffer.isEmpty {
            if !byteBuffer.isEmpty {
                parseAvailableValues(maximumCount: maximumCount)
            }
            ...
            if upstreamFinished {
                ...
                do {
                    return try await body(InputSpan<Value>())
                } catch {
                    throw .second(error)
                }
            }
        ...
        let inputSpan = valueBuffer._consumeAll()
        do {
...
extension ParsingAsyncReader where Upstream: ~Copyable & ~Escapable {
    /// Parses as many complete values as possible (up to `maximumCount`)
    /// from the buffered bytes. Consumes parsed bytes from the front of
    /// the byte buffer. Stops on insufficient data or a parse error.
    private mutating func parseAvailableValues(maximumCount: Int) {
        while valueBuffer.count < maximumCount && !byteBuffer.isEmpty {
    ...
    /// internal byte buffer. Sets `upstreamFinished` on an empty read.
    private mutating func readMoreBytes() async throws {
        try await upstream.read { (inputSpan: consuming InputSpan<UInt8>) in
            if inputSpan.isEmpty {
                self.upstreamFinished = true
                return
            }
            let span = inputSpan.span
